...
Power BI passes parameters to R scripts as a data.frame. A common but unsafe pattern is concatenating those raw string values directly into queries:

```r
# UNSAFE — vulnerable to injection, no type safety
query <- paste0("SELECT * FROM sales WHERE region = '", dataset$Region, "'")
```
